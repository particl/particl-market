...
describe('ImageCompressCommand', () => {
    ...
    test('Should fail to post because missing imageId', async () => {
    ...
    test('Should fail to add because invalid imageId', async () => {
        const res = await testUtil.rpc(imageCommand, [imageCompressCommand,
            false
        ]);
        res.expectJson();
    ...
    test('Should fail to add because invalid messageVersionToFit', async () => {
        const res = await testUtil.rpc(imageCommand, [imageCompressCommand,
    ...
    test('Should fail to add because invalid scalingFraction', async () => {
        const res = await testUtil.rpc(imageCommand, [imageCompressCommand,
            listingItemTemplate.ItemInformation.Images[0].id,
    ...
    test('Should fail to add because invalid qualityFraction', async () => {
        const res = await testUtil.rpc(imageCommand, [imageCompressCommand,
            listingItemTemplate.ItemInformation.Images[0].id,
            CoreMessageVersion.FREE,
            0.9,
    ...
    test('Should fail to add because invalid maxIterations', async () => {
        const res = await testUtil.rpc(imageCommand, [imageCompressCommand,
            listingItemTemplate.ItemInformation.Images[0].id,
            CoreMessageVersion.FREE,
            0.9,
